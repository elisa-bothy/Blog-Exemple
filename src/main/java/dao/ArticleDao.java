    ...
    public Article read(int id) {
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            ...
            if (rs.first()){
                ...
                obj.setAuthor(new PersonDao().readInt(rs.getInt("author")));
            }
     ...
     public Article readContent(String content) {
        Article obj = null;
        String sql = "SELECT * FROM article WHERE content=?";
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            pstmt.setString(1, content);//le premier ? prend la valeur id 
            // que j'ai mis en paramètre
            ResultSet rs = pstmt.executeQuery();
            if (rs.first()){
                obj = new Article();
    ...
    public Collection<Article> list(){
        ...
        try {
            ...
            while(rs.next()){
                ...
                a.setAuthor(new PersonDao().readInt(rs.getInt("author")));
                list.add(a);
            }
        } catch (SQLException ex) {
     ...
     public Collection<Article> listLastN(int qty){
        ArrayList<Article> list = new ArrayList<>();
        String sql = "SELECT * FROM article ORDER BY created DESC LIMIT ?";
        try (PreparedStatement pstmt = connection.prepareStatement(sql)){
            pstmt.setInt(1, qty);
            ResultSet rs = pstmt.executeQuery();
            while(rs.next()){
